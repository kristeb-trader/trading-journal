...
El sistema tiene 6 piezas que trabajan juntas:
...
| **Dashboard Web** | Ver métricas, calendario, galería, análisis, registrar sesión | Al final del día o cuando quieras revisar |
...
**Fusión automática con ATM:** si operas 2 o más contratos usando ATM (el sistema los cierra por separado), el indicador espera 3 segundos acumulando todos los cierres del mismo instrumento y los publica como **un único trade consolidado**. Esto evita duplicados en el journal.
...
7. En la lista de indicadores disponibles, busca **SupabaseAutoExport** (o el nombre que aparezca).
...
4. Cuando cierras una posición, el indicador detecta el trade y lo envía a la nube en segundos (o tras la ventana de fusión de 3 segundos si usas ATM con múltiples contratos).
...
| Trades con ATM aparecen duplicados | Indicador desactualizado (pre-v2.1) | Actualiza al indicador v2.1 que incluye fusión automática de 3 segundos |
...
Un calendario mensual donde cada día con actividad tiene un color. En la parte superior hay un **filtro de cuenta** que por defecto selecciona PA-APEX y persiste entre sesiones.

**Colores del calendario (6 en total):**

| Color | Significado |
|---|---|
| 🟢 Verde | Target alcanzado |
| 🔴 Rojo | Stop activado |
| 🟡 Amarillo | Sin entradas válidas (hubo sesión pero no trade con resultado claro) |
| ⚫ Negro/gris | No operé (motivo personal, noticia, etc.) |
| 🔵 Azul tenue | Festivo CME — mercado cerrado ese día |
| 🟠 Ámbar/caramelo | Día FOMC (no operado por reunión de la Fed) |
...
| 🔺/🔻 flecha | Esquina inferior derecha | Dirección del trade principal (largo/corto) |
| 📊 ámbar pequeño | Esquina superior izquierda | Día FOMC pero sí operaste |

**Festivos CME:** Los festivos se calculan automáticamente para cualquier año (New Year, MLK Day, Presidents' Day, Good Friday, Memorial Day, Juneteenth, Independence Day, Labor Day, Thanksgiving, Christmas). Aparecen con fondo azul tenue y badge "🏦 Festivo" sin necesidad de registrar sesión.

**¿Qué puedes hacer aquí?**

- Ver de un vistazo qué tan consistente fue tu mes.
- Filtrar por cuenta (el filtro persiste al navegar entre meses).
- Hacer clic en cualquier día para ver un panel con 4 pestañas (en este orden):
  1. **Imagen:** la captura del gráfico del día (se muestra primero)
  2. **Análisis:** contexto, setup, reflexión tuya + resumen de la IA
  3. **Checklist:** las 6 reglas con ✅/❌ y tu score del día
  4. **Resumen:** lista de trades del día con ganancia/pérdida
- Navegar entre meses con las flechas. Las métricas se actualizan automáticamente al cambiar de mes.

> 💡 **Tip:** Usa el calendario para identificar patrones: ¿hay días de la semana donde pierdes más? ¿Hay semanas donde la consistencia cae? ¿Los días FOMC afectan tu resultado?
...
Los indicadores clave de rendimiento (KPIs) de tu trading. Siguen el **mes mostrado en el calendario** y el **filtro de cuenta** seleccionado.
...
| Error más frecuente | La casuística (error tipificado) que más se repite |

**Disciplina — 7 factores:**
La métrica de disciplina evalúa 6 ítems del checklist **más** un factor adicional "sin errores de tipificación". Incluye también los días "Sin setup" en el cálculo. El sub-texto muestra "N/total sesiones con fallos".

- 🟢 Verde: ≥ 80%
- 🟡 Amarillo: 50-79%
- 🔴 Rojo: < 50%

**Hacer clic en Disciplina** abre un modal con:
- Barras por factor, ordenadas de más a menos fallos.
- Lista de días con fallos y qué factores fallaron en cada uno.

**Error más frecuente:** muestra la **casuística** más frecuente (tipificaciones de errores que registras en el formulario), no el ítem del checklist. Hacer clic abre un modal con la frecuencia de cada casuística.

> 💡 **Tip:** Revisa las métricas al menos una vez por semana. El "error más frecuente" y el desglose de disciplina te dicen exactamente en qué trabajar.

---

### Sección 1.5: Galería de Imágenes

**¿Qué muestra?**

Una galería visual de todas las capturas de pantalla que has subido, organizadas por semana y con información de contexto en cada miniatura.

**Características:**

- **Filtro de mes** en la parte superior: selecciona el mes que quieres ver.
- **Agrupación por semana:** las miniaturas se organizan en grupos semanales.
- **Borde de color:** cada miniatura tiene un borde que indica el resultado del día:
  - Verde = target alcanzado
  - Rojo = stop activado
  - Amarillo = resultado mixto
- **Iconos en la miniatura:**
  - Flecha de dirección (largo ↑ / corto ↓)
  - Icono de error ⚠️ si hubo casuísticas ese día
- **Días sin imagen:** se muestran como un recuadro vacío con borde punteado. Si es una fecha futura, aparece un icono 🕐.

**Lightbox:**

Al hacer clic en cualquier miniatura, la imagen se abre en pantalla completa con:
- Flechas prev / next para navegar entre imágenes.
- Teclado: ← → para navegar, **Esc** para cerrar.
- Contador en la esquina: "3 / 12".

> 💡 **Tip:** Usa la galería para comparar visualmente días ganadores con días perdedores. A veces el patrón es más obvio cuando lo ves en las imágenes que cuando lo lees en los números.
...
Una tabla con todos tus trades, mostrando para cada uno: fecha, hora, instrumento, dirección (largo/corto), resultado en puntos y resultado en dólares.
...
**Casuísticas (errores de tipificación):**

Las casuísticas son errores específicos que puedes tipificar en cada sesión. El campo de casuísticas **permanece visible aunque marques "No operé hoy"**, porque a veces el error fue no operar cuando debías, o identificar una lección del día sin haber abierto posición.
...
| Sin entradas válidas | No se presentó ningún setup que cumpliera tus criterios |
| Noticia roja activa | Había un evento de alto impacto que te hizo esperar |
| Motivo personal | Razón personal (salud, tiempo, etc.) |
| Otro | Cualquier otra razón |

**Subir imagen:**

Puedes subir una captura de pantalla de tu chart del día. El sistema la guarda automáticamente y la muestra en la Galería de Imágenes.
...
- [ ] Verifica que el gráfico con el indicador SupabaseAutoExport esté abierto y activo.
...
- [ ] Verifica en el calendario si hoy es festivo CME o día FOMC.
...
- [ ] Identifica el error más frecuente de la semana en **Métricas** (clicka en Disciplina para el desglose).
...
Sí. En el calendario, usa las flechas de navegación para ir al mes anterior. Todos los datos históricos están disponibles desde que empezaste a usar el sistema. La galería de imágenes también tiene filtro por mes.

---

**7. ¿Qué hago si un trade aparece duplicado en la Tabla de Trades?**

A partir de v2.1, el indicador fusiona automáticamente trades del mismo instrumento en una ventana de 3 segundos (para trades ATM con múltiples contratos). Si aún aparecen duplicados con el indicador actualizado, reporta el caso al administrador.

---

**8. ¿Qué diferencia hay entre Disciplina y Error más frecuente?**

**Disciplina** evalúa los 6 ítems del checklist + el factor "sin errores de tipificación" (7 en total). Mide si seguiste tus reglas operativas. **Error más frecuente** muestra la casuística específica que más se repite — son errores concretos que tipificaste tú mismo (ej: "entré tarde", "moví el stop"). Haz clic en cualquiera de las dos métricas para ver el desglose detallado.
...
| **Casuística** | Error específico tipificado por el trader (ej: "entré tarde", "no respeté el plan"). Se usa para el cálculo de disciplina y el error más frecuente. |
...
| **Festivo CME** | Día en que el mercado CME (donde se opera el NQ) está cerrado. Se muestra en azul en el calendario. |
| **FOMC** | Federal Open Market Committee. Reunión de la Fed que puede generar alta volatilidad. Se muestra en ámbar en el calendario. |
| **Fusión ATM** | Proceso automático del indicador NT8 que agrupa trades de múltiples contratos ATM en un único registro durante una ventana de 3 segundos. |
| **Lightbox** | Vista de imagen a pantalla completa con navegación prev/next y atajos de teclado, disponible en la Galería. |
...
*Manual de Usuario — Trading Journal NQ Futures | Versión 2.1 | Mayo 2026*
