//
// RR - Risk Reward que mide en PUNTOS
//
// Clon de la herramienta de dibujo "Risk Reward" de NinjaTrader (@RiskReward.cs),
// con una sola diferencia funcional: la etiqueta muestra la distancia en PUNTOS.
//
// POR QUE UN CLON Y NO UNA MODIFICACION:
//   1. @RiskReward.cs lo SOBRESCRIBE NinjaTrader en cada actualizacion: cualquier
//      cambio ahi se pierde solo.
//   2. La original usa el enum ValueUnit, que vive compilado dentro de las DLL de
//      NinjaTrader (Price/Percent/Ticks/Currency/Pips) y NO admite un valor nuevo.
//      Por eso aqui se declara el enum propio RRUnit.
//
// La herramienta original queda INTACTA: conviven las dos.
//
// Instalacion: copiar a  Documentos > NinjaTrader 8 > bin > Custom > DrawingTools
//              y pulsar F5 en el editor de NinjaScript.
...
{
	/// <summary>
	/// Unidades de la etiqueta de RR. Enum PROPIO: el ValueUnit de NinjaTrader vive
	/// en sus DLL y no admite "Points". Solo las dos unidades que se usan.
	/// </summary>
	public enum RRUnit
	{
		Points,
	...
	public class RR : DrawingTool
	{
		...
		private string GetPriceString(double price, ChartBars chartBars)
		{
			...
			switch (DisplayUnit)
			{
				// PUNTOS (por defecto). La distancia entre la linea y la entrada,
				// en unidades de precio. Los decimales los decide el tick del
				// instrumento, con la MISMA formula que usa NinjaTrader para su
				// propia unidad Points (@NetChangeDisplay.cs).
				//   NQ: 41,75 puntos  ==  167 ticks  ==  $835 (a $20/punto)
				case RRUnit.Points:
					priceString = Math.Abs(AttachedTo.Instrument.MasterInstrument.RoundToTickSize(price - yValueEntry))
						.ToString(Core.Globals.GetTickFormatString(tickSize), Core.Globals.GeneralOptions.CurrentCulture);
					break;
				// DINERO. Identico a la herramienta original, rama de Forex incluida.
				case RRUnit.Currency:
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= "Risk Reward que mide la distancia en PUNTOS (por defecto) o en dinero.";
				Name						= "RR";
	...
	{
		private static RR RRCore(NinjaScriptBase owner, string tag,
			bool isAutoScale, 
			...
			double ratio, bool isStop, bool isGlobal, string templateName)
		{
			...
			if (DrawingTool.GetByTagOrNew(owner, typeof(RR), tag, templateName) is not RR rr)
				return null;

			DrawingTool.SetDrawingToolCommonValues(rr, tag, isAutoScale, owner, isGlobal);
			...
			if (isStop)
			{
				...
				entryAnchor.CopyDataValues(rr.EntryAnchor);
				entryAnchor.CopyDataValues(rr.RewardAnchor);
				stopAnchor.CopyDataValues(rr.RiskAnchor);
				rr.SetReward();
			}
			else 
			{
				...
				entryAnchor.CopyDataValues(rr.EntryAnchor);
				entryAnchor.CopyDataValues(rr.RiskAnchor);
				targetAnchor.CopyDataValues(rr.RewardAnchor);
				rr.SetRisk();
			}
			
			rr.SetState(State.Active);
			return rr;
		}
		...
		/// <returns></returns>
		public static RR RR(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, bool isStop) =>
			isStop
				? RRCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, true, false, null)
				: RRCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, false, false, null);
		...
		/// <param name="isStop">A bool value, when true will use the endTime/endBarsAgo and endY to set the stop, and will automatically calculate the target based off the ratio value.</param>
		/// <returns></returns>
		public static RR RR(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, bool isStop) =>
			isStop
				? RRCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, true, false, null)
				: RRCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, false, false, null);
		...
		/// <returns></returns>
		public static RR RR(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, bool isStop, bool isGlobal, string templateName) =>
			isStop
				? RRCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, true, isGlobal, templateName)
				: RRCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, false, isGlobal, templateName);
		...
		/// <param name="templateName">The name of the drawing tool template the object will use to determine various visual properties</param>
		/// <returns></returns>
		public static RR RR(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, bool isStop, bool isGlobal, string templateName) =>
			isStop
				? RRCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, true, isGlobal, templateName)
				: RRCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, false, isGlobal, templateName);
	}
}