...
const reglas = JSON.parse(leer('reglas.json'))
const ids = new Set(reglas.map((r) => r.id))
const DOCS = fs.readdirSync(PLAN).filter((f) => f.endsWith('.md'))
// Los documentos que describen el plan de HOY. TRADING_PLAN_CHAUMER.md, GALERIA.md, PENDIENTES.md y
...
function camposDe(r) {
  const out = [['enunciado', r.enunciado], ['accion', r.accion], ['nota', r.nota], ['estado', r.estado]]
  for (const c of r.condiciones || []) out.push([`condiciones.${c.variable}`, c.valor])
  for (const e of r.excepciones || []) out.push(['excepciones', e])
...
{
  const fuentes = [['reglas.json', JSON.stringify(reglas)], ...VIVOS.map((f) => [f, leer(f)])]
...
const VALORES = [
  ...
  ['PLAZO_CONSECUCION', /\b5 velas\b/gi, /\*\*5 velas/],
...
}
{
  ...
  for (const r of reglas) {
    for (const [campo, texto] of camposDe(r)) {
      if (campo === 'estado' || campo === 'nota') continue   // la historia se mira en el apartado 6
      for (const [nombre, patron] of VALORES) {
        const n = (texto.match(patron) || []).length
        if (n) suma(`${nombre} · reglas.json (${r.id})`, n)
      }
    }
  }
...
// ── 4 · Parámetros que no existen ────────────────────────────────────────────
{
  ...
  const fuentes = [['reglas.json', JSON.stringify(reglas)], ['CHECKLIST_DIARIA.md', leer('CHECKLIST_DIARIA.md')]]
  for (const [f, t] of fuentes) {
...
// ── 5 · Versiones y cuentas ──────────────────────────────────────────────────
{
  ...
  for (const [f, t] of cabeceras) {
    ...
    if (mal.length) anota('5 · Versiones y cuentas', `${f} dice ${mal.join(' y ')} reglas; reglas.json tiene ${reglas.length}`)
  }
}

// ── 6 · Historia dentro de las reglas ────────────────────────────────────────
{
  const FECHA = /\b\d{1,2}\/\d{2}\/20\d{2}\b|\b20\d{2}-\d{2}-\d{2}\b/g
  const MARCAS = /palabras del operador|\bmotor\b|regresi[oó]n|\bampliad[ao]\b|\breescrit[ao]\b|\bcorregid[ao]\b/gi
...
// ── 7 · Palabras sin tilde en reglas.json ────────────────────────────────────
{
  ...
  const n = (JSON.stringify(reglas).match(SIN_TILDE) || []).length
  if (n) anota('7 · Palabras sin tilde en reglas.json', `${n} apariciones de palabras comunes sin tilde («grafico», «sesion», «continuacion»…)`)
}

// ── Informe ──────────────────────────────────────────────────────────────────
console.log(`Vigilante del plan · ${reglas.length} reglas · modo ${ESTRICTO ? 'estricto' : 'informe'}\n`)
if (!hallazgos.length) {
...
console.log(`\n${hallazgos.length} hallazgos en ${apartados} de 7 apartados.`)
